...
//Manager::Manager(Pipe & graphicsPipe) use this line when using graphics
...
void Manager::changeTurn()
{
	...
	_otherPlayer = temp;
	_brd->delLastPieceIfGarbage();	 //delete last piece that was eaten 
}
...
int Manager::playMove(string move)
{
	...
	if (srcRow < 0 || srcRow >= BOARD_SIZE || //boundries check
	...
	if (!_brd->isPieceOfPlayer(srcRow, srcCol, _currPlayer)) //ownerships check
	{
		return INVALID_NOT_YOUR_PLAYER;
	}

	if (_brd->isPieceOfPlayer(dstRow, dstCol, _currPlayer)) //if dest has friend
	{
		return INVALID_DEST_NOT_FREE;
	}


	if (srcRow == dstRow && srcCol == dstCol) //null move check
	{
		return INVALID_SRC_AND_DEST_EQUAL;
	}

	if (!_brd->tryMove(srcRow, srcCol, dstRow, dstCol)) //try the move
	{
	...
	// check for chess
	if (_currPlayer->getKing()->isChess())  // - activate when chess checking
	{
		
		_brd->print();
		_brd->undoLastMove();				// if chess undo the last move
	...
	if (_otherPlayer->getKing()->isChess())		// after move check if rival is in chess
	{
	...
	//if (dstRow == 0 && _currPlayer->isGoingUp() || dstRow == (BOARD_SIZE - 1) && !(_currPlayer->isGoingUp()))
	//	_brd->crownThePawn(dstRow, dstCol);  **algorithm works but display won't change - check why**
...
void Manager::playGame()
{
	...
	try {
		...
		while (msgFromGraphics != "quit")
		{
			...
			int r = playMove(msgFromGraphics);			// according the protocol. Ex: e2e4           (move e2 to e4)
			msgToGraphics[0] = (char)(r + '0');
			msgToGraphics[1] = 0;

				
			_pipe.sendMessageToGraphics(msgToGraphics);		// return result to graphics	
